...
# Step 5: Duplicate photos in _resH to have exactly 693 photos with random alphanumeric prefixes
...
# Function to generate a random 3-character alphanumeric string using a predefined set of safe characters
generate_random_prefix() {
    chars=({A..Z} {a..z} {0..9})
    prefix=""
    for i in {1..3}; do
        prefix+="${chars[RANDOM % ${#chars[@]}]}"
    done
    echo "$prefix"
}

echo "Duplicating photos in _resH to have exactly 693 photos with random alphanumeric prefixes..."
